...
public class SchoolController {
	...
	public String load_school(Model model){

		return "registerTwo.do";
	}
	
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@ResponseBody
	@RequestMapping(value = "/loadschool", method = RequestMethod.POST)
	public List<School> load_school(@RequestParam("school_name")String school_name){
		List list=new ArrayList<School>();
		load();
		for(String addr : allschool.keySet()){
        	String schools[]=allschool.get(addr);  //获取所有学校
        	for(int i=0;i<schools.length;i++){
        		if(schools[i].contains(school_name)){
        			School s=new School();
        			s.setS_name(schools[i]);
        			list.add(s);
        			System.out.println("阿迪斯射点"+schools[i]);
        		}
        	}
        }		
