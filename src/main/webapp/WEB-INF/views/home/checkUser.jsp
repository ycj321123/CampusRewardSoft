...
<body>
	<div class="header">
		<div class="top">
			<div class="address" onclick="toCharPage(${user.getUaccount()},'${id}','${isto}');">&lt;</div>
	...
	<div class="footer">
		...
		<c:choose>
			<c:when test="${gr.getIsAgree()==0}">
				<div class="option0"
					onclick="noFr(${user.getUaccount()},'${id}','${isto}')">
					拒绝请求</div>
				<div class="option1"
					onclick="add(${user.getUaccount()},'${id}','${isto}')">
					同意请求</div>
			</c:when>
			<c:when test="${gr.getIsAgree()==2}">
				<div class="option0" style="width: 100%;" >
					已拒绝申请</div>	
			</c:when>
			<c:when test="${gr.getIsAgree()==1}">
				<div class="option1" style="width: 100%;">
					已同意申请</div>
			</c:when>
			<c:otherwise>
			<div class="option1" style="width: 100%;">
	...
	<script>
		function toCharPage(a,b,c){
			if(c=="440")window.location.href="/CampusRewardSoft/mainIndex/a100000/2/440/chatPage.do";
			else
			window.location.href="/CampusRewardSoft/mainIndex/"+a+"/"+b+"/"+c+"/chatPage.do";
		}
		//拒绝
		function noFr(a,b,c){
			window.location.href="/CampusRewardSoft/mainIndex/"+a+"/"+b+"/"+c+"/nofriend.do";
		}
		//同意
		function add(a,b,c){
			window.location.href="/CampusRewardSoft/mainIndex/"+a+"/"+b+"/"+c+"/agreeAdd.do";
		}
