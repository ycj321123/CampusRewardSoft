	...
	<c:if test="${state=='点击验证'}">
		<div class="footer">
			<div class="option1" style="background: #339aff;"
				onclick="testJob(3,'${uj.getPublish_job().getPId()}')">没有完成</div>
			<div class="option2" style="background: #FF6700;"
				onclick="testJob(2,'${uj.getPublish_job().getPId()}')">确认完成</div>
		</div>
	</c:if>
	<script>
		function testJob(a,b){		//检验任务
			if(a==3){
				if(confirm("确定任务失败吗？")){
					$.ajax({
						type : "POST",
						url : "/CampusRewardSoft/mainIndex/"+b+"/testJob2.do",
						data : {result:"3"},
						dataType : "text",
						success : function(data) {
							console.log(data);
							window.location.href="/CampusRewardSoft/mainIndex/"+data+"/2/myJobPage.do";
						},
						error : function(e) {
							alert("发布错误！！");
						}
