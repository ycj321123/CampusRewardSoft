		...
		<div class="sidebar">
			<div class="scrollbar-inner sidebar-wrapper">
				...
				<ul class="nav">
					...
					<li class="nav-item active" id="jobli"><a> <i
					...
					<ul class="nav" style="color: #000; margin: 0px" id="jobnav">
							...
							<li class="newli"><a href="jobTuiJianManager.do" style="color: #339aff;">任务推荐</a></li>
						</ul>
					</li>
				...
				<div class="container-fluid">
					...
					<div class="card">
						<div class="card-header">
							<div class="card-title">任务推荐</div>
						</div>
						<div id="lookText" style="border-radius:10px; border:1px solid #ccc; display:none; position: fixed; top: 100px;right:15%;height:500px; float: left;width:50%;background: #ffa;padding: 5px; overflow: scroll;"><span id="closeText" style="float: right;padding: 5px;background: red;">关闭</span><h7 id="title" style="color:#000;font-size:20px;">asdfasfafa</h7><div id="content">asdfgasfiashui</div><div id="textimg" style="margin-top: 5px;"></div></div>
						<div class="card-body">
							<div class="table-responsive">
								<div style="margin-bottom: 15px;">
									关键字搜索：<input type="text" id="search" style="width: 22%;"
										placeholder="搜索" style=" padding: 5px;" value="${name}"
										onchange="search()" />
								</div>
									...
									<tbody>
										<c:forEach items="${page.getDatas()}" var="page" varStatus="s">
											<tr>
												...
												<td>${page.getPublish_job().getpAccount()}</td>
												<td>${page.getPublish_job().getPId()}</td>
												<td style="color: #339aff;"
													onclick="lookjob('${page.getPublish_job().getPId()}')">查看</td>
												<td>${page.getPublish_job().getTimeLimit()}</td>
												<td>${page.getPublish_job().getPrice()}</td>
												<td>${page.getPublish_job().getpTime()}</td>
												<td>
												<c:choose>
												<c:when test="${page.getPublish_job().getJobLevel()==0}">其他</c:when>
												<c:when test="${page.getPublish_job().getJobLevel()==1}">跑腿</c:when>
												<c:when test="${page.getPublish_job().getJobLevel()==2}">快递</c:when>
												<c:when test="${page.getPublish_job().getJobLevel()==3}">外卖</c:when>
												<c:otherwise>人类</c:otherwise>
												</c:choose>
												</td>
												<c:choose>
													<c:when test="${page.getOutTime()=='已完成'}">
														<td style="color:green">${page.getOutTime()}</td>
													</c:when>
													<c:when test="${page.getOutTime()=='失败了'}">
														<td style="color:red">${page.getOutTime()}</td>
													</c:when>
													<c:when test="${page.getOutTime()=='待验证'}">
														<td style="color:orange">${page.getOutTime()}</td>
													</c:when>
													<c:when test="${page.getOutTime()=='进行中'}">
														<td style="color:#339aff;">${page.getOutTime()}</td>
													</c:when>
													<c:otherwise>
														<td style="color:#555">${page.getOutTime()}</td>
													</c:otherwise>
												</c:choose>
												<td><a onclick="tuijian('${page.getPublish_job().getPId()}')" style="color:#399aff;">推荐给用户</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
