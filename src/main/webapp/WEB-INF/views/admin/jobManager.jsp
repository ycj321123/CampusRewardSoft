			...
			<div class="scrollbar-inner sidebar-wrapper">
				...
				<ul class="nav">
					...
					<ul class="nav" style="color: #000; margin: 0px" id="jobnav">
							<li class="newli" ><a href="jobManager.do" style="color: #339aff;">任务发布审核</a></li>
				...
				<div class="container-fluid">
					...
					<div class="card">
						...
						<div class="card-body">
							...
							<div style="margin-bottom: 15px;">关键字搜索：<input type="text" id="search" style="width: 22%;" placeholder="搜索"  style=" padding: 5px;" value="${name}" onchange="search()" /></div>
									...
									<tbody>
										<c:forEach items="${page.getDatas()}" var="page" varStatus="s">
											<tr style="text-align: center;">
												...
												<td>${page.getpAccount()}</td>
												<td>${page.getPId()}</td>
												<td style="color:#339aff;" onclick="lookjob('${page.getPId()}')">查看</td>
												<td>${page.getTimeLimit()}</td>
												<td>${page.getPrice()}</td>
												<td>${page.getpTime()}</td>
												<td>
												<c:choose>
												<c:when test="${page.getJobLevel()==0}">其他</c:when>
												<c:when test="${page.getJobLevel()==1}">跑腿</c:when>
												<c:when test="${page.getJobLevel()==2}">快递</c:when>
												<c:when test="${page.getJobLevel()==3}">外卖</c:when>
												<c:otherwise>人类</c:otherwise>
												</c:choose>
												</td>
												<td><a style="border: 2px solid red;" onclick="pass('${page.getPId()}',0)">不通过</a> &nbsp; &nbsp; <a style="border: 2px solid #339aff;" onclick="pass('${page.getPId()}',1)">通过</a></td>
											</tr>
										</c:forEach>
									</tbody>
								</table>
								<div class="pageUl">
									<ul>
										...
										<li style="float: left;background: #fff;">跳转到第<input type="text" size="3" id="trans" oninput="value=value.replace(/[^\d]/g,'')">页 <span style="background: #339aff; padding: 4px;" onclick="tran(${page.getPageEnd()})">跳转</span>&nbsp;&nbsp;&nbsp; &nbsp; &nbsp;总页数：${page.getTotalPage() }</li>
...
function pass(a,b){
	$.ajax({
		...
		dataType : "text",
		success : function(data) {
			window.location.href="jobManager.do";
		},
		error : function(e) {
			alert("错误！！");
		}
