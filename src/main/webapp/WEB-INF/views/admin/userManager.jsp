	...
	<div class="wrapper">
					...
					<ul class="navbar-nav topbar-nav ml-md-auto align-items-center">
						...
						<li class="nav-item dropdown"><a
							...
							href="#" aria-expanded="false"> <img
								...
								alt="user-img" width="36" class="img-circle"><span>${ui.getNickname()}</span>
						</a>
		...
		<div class="sidebar">
			<div class="scrollbar-inner sidebar-wrapper">
				...
				<ul class="nav">
					<li class="nav-item active" id="userli"><a> <i
							class="la la-table"></i>
							<p>用户管理</p>
					</a>
					<ul class="nav" style="color: #000;margin: 0px" id="newnav">
						<li class="newli">
						<a href="userManager.do" style="color: #339aff;">用户注册表</a>
						</li>
						<li class="newli">
				...
				<div class="container-fluid">
					<div class="card">
						...
						<div class="card-body">
							...
							<div style="margin-bottom: 15px;">关键字搜索：<input type="text" id="search" placeholder="搜索手机号、昵称、邮箱" style=" padding: 5px;" value="${name}" onchange="search()" /></div>
									...
									<tbody>
										<c:forEach items="${page.getDatas()}" var="page" varStatus="s">
											<tr>
												...
												<td>${page.getEmail()}</td>
												<td>${page.getPassword()}</td>
												<td>${page.getCreditIntegral()}</td>
												<c:choose>
												<c:when test="${page.getRole()==0}"><td>用户</td></c:when>
												<c:when test="${page.getRole()==1}"><td>管理</td></c:when>
												<c:otherwise><td style="color:red;">被封</td></c:otherwise>
												</c:choose>
												<c:choose>
												<c:when test="${page.getRole()==4}"><td><a onclick="jiejin('${page.getAccount()}','0',this)">解封</a></td></c:when>
												<c:when test="${page.getRole()==1}"><td></td></c:when>
												<c:otherwise><td><a onclick="jiejin('${page.getAccount()}','1',this)" style="color:green;">封号</a></td></c:otherwise>
												</c:choose>
											</tr>
										</c:forEach>
									</tbody>
